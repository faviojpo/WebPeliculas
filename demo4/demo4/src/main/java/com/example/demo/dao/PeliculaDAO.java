...
public class PeliculaDAO {
    ...
    public List<Pelicula> listar() {
        try {
            ...
            // Se recomienda trabajar con las palabras reservadas de SQL
            // En mayus y las tablas y campos en minus
            ResultSet rs=st.executeQuery("SELECT * FROM peliculas2022");
    ...
    public List<Pelicula> buscar(String titulo) {
        try {
            ...
            Statement st=ConnectorDB.getSt();
            ResultSet rs=st.executeQuery("Select * from peliculas2022 where titulo like '%"+titulo+"%'");
    ...
    public Boolean add(Pelicula peli) {
        try {
            Statement st=ConnectorDB.getSt();
            Integer cantInsert=st.executeUpdate("INSERT INTO peliculas2022 (titulo, puntuacion, portada) VALUES ('"+peli.getTitulo()+"', "+peli.getPuntuacion()+", '"+peli.getPortada()+"')");
    ...
    public Boolean del(Pelicula peli) {
        try {
            Statement st=ConnectorDB.getSt();
            Integer cantDel=st.executeUpdate("DELETE FROM peliculas2022 WHERE id="+peli.getId());
